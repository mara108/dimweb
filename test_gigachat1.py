...
def generate_via_generator_rules(word: str, max_per_category: int = 3) -> Dict[str, List[str]]:
    """Делегирует генерацию в уже проверенную (через OpenCorpora) логику
    generator.py вместо ручных строковых шаблонов PATTERN_RULES.
    Приоритет отдаём формам, подтверждённым словарём (in_opencorpora),
    но не отбрасываем остальные — как и в build_dictionary_free.py,
    отсутствие в словаре не значит, что формы не существует, просто
    сигнал слабее. "неизвестно"/"неоднозначно" (connotation_hint) не
    маппятся ни в одну категорию — не гадаем, что это, оставляем пустым."""
    candidates = gen_candidates(word)
    candidates_sorted = sorted(candidates, key=lambda c: not c.get("in_opencorpora", False))

    result: Dict[str, List[str]] = {"уменьшительные": [], "ласкательные": [], "пренебрежительные": []}
    for c in candidates_sorted:
        category = CONNOTATION_TO_CATEGORY.get(c["connotation_hint"])
        if not category or len(result[category]) >= max_per_category:
            continue
        if c["form"] not in result[category]:
            result[category].append(c["form"])
    return result
